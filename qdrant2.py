...
# def analyze_cv_pnl(cv_text, language='en'):
#     """
#     Realiza un análisis básico de PNL del CV utilizando spaCy.
...
def clean_and_merge_text(text):
    ...
    # Elimina espacios en blanco al inicio y al final, luego divide el texto en palabras
    words = text.strip().split()
    # Une las palabras en una cadena de texto, separadas por un espacio
...
def main():
    ...
    if uploaded_file is not None:
        ...
        #             result = extract_named_entities_spacy(text)  # Usando la función actualizada
        #             entities = result["entities"]  # Accediendo a las entidades nombradas
            ...
            with st.spinner('Extrayendo texto del PDF...'):
                text = get_pdf_text(uploaded_file)[:512]  # Limitando texto para demostración
                clean_text = clean_and_merge_text(text)  # Limpia el texto antes de la extracción de entidades
                st.success("Texto extraído y limpiado correctamente.")

            if ner_method == "Hugging Face":
                with st.spinner('Extrayendo entidades nombradas con Hugging Face...'):
                    entities = extract_named_entities_hf(clean_text)
            elif ner_method == "spaCy":
                with st.spinner('Extrayendo entidades nombradas con spaCy...'):
                    result = extract_named_entities_spacy(clean_text)  # Usa el texto limpio
                    entities = result["entities"]
                    tokens = result["tokens"]  # Si quieres mostrar los tokens también

            if entities:
                st.write("Entidades encontradas:")
                for entity in entities:
                    st.write(f"Texto: {entity['text']}, Tipo: {entity['label']}")
                # Mostrar tokens si se usó spaCy
                if ner_method == "spaCy":
                    st.write("Tokens encontrados:", ', '.join(tokens))
            else:
